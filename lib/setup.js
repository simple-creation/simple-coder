...
const  prepareCompnentsCode = async (template,workRootPath) =>{
    ...
    if (fs.existsSync(frameworkTempPath)){
    ...
    // console.log('current git url',frameworkTemplate.url)
    // console.log('current target path',frameworkTempPath)
...
const obtainProjectConfigData = async (template, projectName) => {
    ...
    if (template.type && (template.type=="frontend" || template.type=="web" || template.type=="ios" || template.type=="android" ||template.type=="h5" ||template.type=="mini")){
        inputParams.push( {
            type: "input",
