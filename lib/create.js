...
const setupProjectInfo = async (template, projectName,config) => {
    ...
    if (template.name =='service:spring-boot'){
        ...
        let replaceMap = {};
        replaceMap["java-template-service"] = projectName;
        replaceMap["service-java-template-parent"] = projectName+ "-parent";
        replaceMap["service-java-template"] = projectName;
        const targetPOMfile = path.join(process.cwd(),projectName,"service/pom.xml");
        const targetParentPOMfile = path.join(process.cwd(),projectName,"pom.xml");
        codeTools.replaceFileContent(targetPOMfile,replaceMap);
