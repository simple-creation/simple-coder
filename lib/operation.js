...
const initProjectByTemplateWithoutCompnents = async (template, configData, workRootPath) => {
    let result = true;
    //const appliationName = configData.name;
...
const setupProjectInfo = async (template, configData, workRootPath) => {
    ...
    if (configData.basePath) {
        replaceMap["{BASE_PATH}"] = applicationName;
    ...
    if (template.id == 'web-antd-pro'|| template.id =='antd-pro') {
    ...
    if (template.id == 'web-nextjs' || template.id=='nextjs') {
...
const createProjectInConsole = async (projectName, templateName, configData, projectRootPath) => {
    ...
    const result = await fetchTemplateCode(template,workRootPath);
    if (!result){
        return false;
    }
    if (!configData){
    ...
    return await setupProjectByTemplate(template, configData, workRootPath);
...
module.exports.initProjectByTemplateWithoutCompnents = initProjectByTemplateWithoutCompnents;
